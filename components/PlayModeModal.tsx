...
// Updated to accept isAuthenticated prop.
// If user is already logged in, it skips the auth screens entirely.
...
export default function PlayModeModal({ isOpen, onClose, isAuthenticated = false }: PlayModeModalProps) {
  ...
  const renderContent = () => {
    switch (currentScreen) {
      case 'mode-select':
        return (
          <div className="space-y-6">
            <h2 className="text-3xl font-black text-white text-center mb-8">Choose Your Game Mode</h2>

            <div
              onClick={() => {
                // If already logged in, skip auth and go straight to game
            ...
            >
              ...
              <h3 className="text-2xl font-black text-white mb-2 text-center">SOLO PLAY</h3>
              <p className="text-gray-300 text-center mb-4">Practice alone and improve your skills</p>
              <button className="w-full py-3 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl transition-colors">
                {isAuthenticated ? 'Play Now →' : 'Play Solo →'}
              </button>
            </div>
            ...
            >
              <div className="text-5xl mb-4 text-center">🏆</div>
              <h3 className="text-2xl font-black text-white mb-2 text-center">TOURNAMENTS</h3>
              <p className="text-gray-300 text-center mb-4">Compete in real-time battles</p>
      ...
      case 'solo-auth':
        return (
          <div className="space-y-6">
            <button onClick={() => setCurrentScreen('mode-select')} className="text-gray-400 hover:text-white transition-colors mb-4">← Back</button>
            <h2 className="text-3xl font-black text-white mb-6">Solo Play</h2>
            ...
            <div className="pt-4 border-t border-gray-700">
              ...
              <input
                type="text" placeholder="Enter your username" value={guestUsername}
              ...
              />
              <button onClick={handleGuestPlay} disabled={loading} className="w-full py-3 bg-gray-700 hover:bg-gray-600 text-white font-bold rounded-xl transition-colors disabled:opacity-50">
                Play as Guest
              </button>
              <p className="text-xs text-gray-500 mt-2 text-center">Guest scores won't be saved to leaderboard</p>
            </div>
          </div>
        );
      ...
      case 'tournament-action':
        return (
          <div className="space-y-6">
            <button onClick={() => setCurrentScreen('mode-select')} className="text-gray-400 hover:text-white transition-colors mb-4">← Back</button>
            <h2 className="text-3xl font-black text-white mb-6">Tournament Mode</h2>
            <div className="space-y-4">
              <button
                onClick={() => setCurrentScreen('tournament-join')}
                className="w-full p-6 bg-gradient-to-r from-blue-900/50 to-cyan-900/50 border-2 border-blue-500 rounded-xl hover:scale-105 transition-transform text-left"
              >
                <div className="text-4xl mb-2">🎯</div>
                <h3 className="text-xl font-black text-white mb-2">Join Tournament</h3>
                <p className="text-gray-300 text-sm">Enter a tournament code to join</p>
              </button>
              <button
                onClick={() => {
                ...
                className="w-full p-6 bg-gradient-to-r from-green-900/50 to-emerald-900/50 border-2 border-green-500 rounded-xl hover:scale-105 transition-transform text-left"
              >
                <div className="text-4xl mb-2">➕</div>
                <h3 className="text-xl font-black text-white mb-2">Create Tournament</h3>
                <p className="text-gray-300 text-sm">Host your own tournament</p>
              </button>
            </div>
          </div>
        );
      ...
      case 'tournament-join':
        return (
          <div className="space-y-6">
            <button onClick={() => setCurrentScreen('tournament-action')} className="text-gray-400 hover:text-white transition-colors mb-4">← Back</button>
            <h2 className="text-3xl font-black text-white mb-6">Join Tournament</h2>
            {error && <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg"><p className="text-red-400 text-sm">{error}</p></div>}
            <div>
              <label className="block text-white font-semibold mb-2">Tournament Code</label>
              <input
                type="text" placeholder="Enter 6-character code" value={tournamentCode}
              ...
              />
              <p className="text-xs text-gray-400 mt-2 text-center">{tournamentCode.length}/6 characters</p>
            </div>

            {/* If already logged in, show a direct join button */}
            {isAuthenticated ? (
              <button
                onClick={() => { router.push(`/tournament/join?code=${tournamentCode}`); onClose(); }}
                disabled={tournamentCode.length < 6}
                className="w-full py-4 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-bold rounded-xl transition-colors disabled:opacity-50"
              >
                Join Tournament →
              </button>
            ) : (
              <>
                ...
                <div className="pt-4 border-t border-gray-700">
                  ...
                  <input
                    type="text" placeholder="Enter your username" value={guestUsername}
                    onChange={(e) => setGuestUsername(e.target.value)} maxLength={20}
                    className="w-full px-4 py-3 bg-black/50 border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 mb-4"
                  />
                  <button
                  ...
                  <p className="text-xs text-gray-500 mt-2 text-center">⚠️ Guest players can participate but scores may not count</p>
                </div>
              </>
            )}
          </div>
      ...
      case 'tournament-auth':
        return (
          <div className="space-y-6">
            <button onClick={() => setCurrentScreen('tournament-action')} className="text-gray-400 hover:text-white transition-colors mb-4">← Back</button>
            <h2 className="text-3xl font-black text-white mb-6">Create Tournament</h2>
            <p className="text-gray-400 mb-6">Sign in to create and host your own tournament</p>
            {error && <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg"><p className="text-red-400 text-sm">{error}</p></div>}
            <OAuthButtons redirectTo="/tournaments?action=create" />
            <p className="text-xs text-gray-500 text-center pt-4">Creating tournaments requires authentication</p>
          </div>
        );
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 border-2 border-purple-500/30 rounded-2xl max-w-lg w-full p-8 shadow-2xl max-h-[90vh] overflow-y-auto relative">
        <button
          onClick={handleClose}
